# Press Freedom

There are 1579 Non-NA responses to the Press Freedom question. 

## Islamists

### Islamists from V19

```{r pf_i_v19}

data %>% 
  #mutate(across(starts_with("islamist_"), ~case_when(.x == 0 ~ "Non-Islamist", .x == 1 ~ "Islamist"))) %>%
  group_by(islamist_v19, prime) %>%
  drop_na(press_freedom) %>% 
  summarize(avg = mean(press_freedom), se = sd(press_freedom)/sqrt(length(press_freedom))) %>%
  ggplot(aes(x = prime, y = avg, ymin = avg - se, ymax = avg + se, color = islamist_v19)) +
  geom_errorbar(position = position_dodge(width = 0.2), size=1) +
  labs(x = "", y = "Press Freedom (0-1)", color = "Islamist? V19") +
  ylim(c(0, 1)) +
  scale_color_manual(
    labels = c("0" = "Non-Islamist", "1" = "Islamist"), 
    values = c("#008FB3", "#F36E21")) +
  theme(text = element_text(size=15, family = "Lato", color = "black"), 
        plot.title = element_text(hjust = 0.5, margin = margin(t = 0, r = 0, b = 20, l = 0)),
        axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, color = "black"),
        axis.line = element_line(colour = "black"),
        panel.background = element_rect(fill = "transparent"), # bg of the panel
        panel.grid.major.y = element_line(color = "lightgrey"),
        plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
        legend.background = element_rect(fill = "transparent"), # get rid of legend bg
        legend.box.background = element_rect(fill = "transparent")) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))

```

### Islamists from V25

```{r pf_i_v25}

data %>% 
  #mutate(across(starts_with("islamist_"), ~case_when(.x == 0 ~ "Non-Islamist", .x == 1 ~ "Islamist"))) %>%
  group_by(islamist_v25, prime) %>%
  drop_na(press_freedom) %>% 
  summarize(avg = mean(press_freedom), se = sd(press_freedom)/sqrt(length(press_freedom))) %>%
  ggplot(aes(x = prime, y = avg, ymin = avg - se, ymax = avg + se, color = islamist_v25)) + 
  geom_errorbar(position = position_dodge(width = 0.4), size=1) +
  labs(x = "", y = "Press Freedom (0-1)", color = "Islamist? V25") +
  ylim(c(0, 1)) +
  scale_color_manual(
    labels = c("0" = "Non-Islamist", "1" = "Islamist"), 
    values = c("#008FB3", "#F36E21")) +
  theme(text = element_text(size=15, family = "Lato", color = "black"), 
        plot.title = element_text(hjust = 0.5, margin = margin(t = 0, r = 0, b = 20, l = 0)),
        axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, color = "black"),
        axis.line = element_line(colour = "black"),
        panel.background = element_rect(fill = "transparent"), # bg of the panel
        panel.grid.major.y = element_line(color = "lightgrey"),
        plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
        legend.background = element_rect(fill = "transparent"), # get rid of legend bg
        legend.box.background = element_rect(fill = "transparent")) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))


```

### Islamists from V27

```{r pf_i_v27}

data %>% 
  #mutate(across(starts_with("islamist_"), ~case_when(.x == 0 ~ "Non-Islamist", .x == 1 ~ "Islamist"))) %>%
  group_by(islamist_v27, prime) %>%
  drop_na(press_freedom) %>% 
  summarize(avg = mean(press_freedom), se = sd(press_freedom)/sqrt(length(press_freedom))) %>%
  ggplot(aes(x = prime, y = avg, ymin = avg - se, ymax = avg + se, color = islamist_v27)) + 
  geom_errorbar(position = position_dodge(width = 0.4), size=1) +
  labs(x = "", y = "Press Freedom (0-1)", color = "Islamist? V27") +
  ylim(c(0, 1)) +
  scale_color_manual(
    labels = c("0" = "Non-Islamist", "1" = "Islamist"), 
    values = c("#008FB3", "#F36E21")) +
  theme(text = element_text(size=15, family = "Lato", color = "black"), 
        plot.title = element_text(hjust = 0.5, margin = margin(t = 0, r = 0, b = 20, l = 0)),
        axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, color = "black"),
        axis.line = element_line(colour = "black"),
        panel.background = element_rect(fill = "transparent"), # bg of the panel
        panel.grid.major.y = element_line(color = "lightgrey"),
        plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
        legend.background = element_rect(fill = "transparent"), # get rid of legend bg
        legend.box.background = element_rect(fill = "transparent")) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))


```

### Islamists from V15

```{r pf_i_v15}

data %>% 
  #mutate(across(starts_with("islamist_"), ~case_when(.x == 0 ~ "Non-Islamist", .x == 1 ~ "Islamist"))) %>%
  group_by(islamist_v15, prime) %>%
  drop_na(press_freedom) %>% 
  summarize(avg = mean(press_freedom), se = sd(press_freedom)/sqrt(length(press_freedom))) %>%
  ggplot(aes(x = prime, y = avg, ymin = avg - se, ymax = avg + se, color = islamist_v15)) + 
  geom_errorbar(position = position_dodge(width = 0.4), size=1) +
  labs(x = "", y = "Press Freedom (0-1)", color = "Islamist? V15") +
  ylim(c(0, 1)) +
  scale_color_manual(
    labels = c("0" = "Non-Islamist", "1" = "Islamist"), 
    values = c("#008FB3", "#F36E21")) +
  theme(text = element_text(size=15, family = "Lato", color = "black"), 
        plot.title = element_text(hjust = 0.5, margin = margin(t = 0, r = 0, b = 20, l = 0)),
        axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, color = "black"),
        axis.line = element_line(colour = "black"),
        panel.background = element_rect(fill = "transparent"), # bg of the panel
        panel.grid.major.y = element_line(color = "lightgrey"),
        plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
        legend.background = element_rect(fill = "transparent"), # get rid of legend bg
        legend.box.background = element_rect(fill = "transparent")) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))


```

## Democrats

```{r pf_d_v24}

data %>% 
  #mutate(across(starts_with("democrat_"), ~case_when(.x == 0 ~ "Non-Democrat", .x == 1 ~ "Democrat"))) %>%
  group_by(democrat_v24, prime) %>%
  drop_na(press_freedom) %>% 
  summarize(avg = mean(press_freedom), se = sd(press_freedom)/sqrt(length(press_freedom))) %>%
  ggplot(aes(x = prime, y = avg, ymin = avg - se, ymax = avg + se, color = democrat_v24)) + 
  geom_errorbar(position = position_dodge(width = 0.4), size=1) +
  labs(x = "", y = "Press Freedom (0-1)", color = "Democrat? V24") +
  ylim(c(0, 1)) +
  scale_color_manual(
    labels = c("0" = "Non-Democrat", "1" = "Democrat"), 
    values = c("#008FB3", "#F36E21")) +
  theme(text = element_text(size=15, family = "Lato", color = "black"), 
        plot.title = element_text(hjust = 0.5, margin = margin(t = 0, r = 0, b = 20, l = 0)),
        axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, color = "black"),
        axis.line = element_line(colour = "black"),
        panel.background = element_rect(fill = "transparent"), # bg of the panel
        panel.grid.major.y = element_line(color = "lightgrey"),
        plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
        legend.background = element_rect(fill = "transparent"), # get rid of legend bg
        legend.box.background = element_rect(fill = "transparent")) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))


```

## Anti-Americans

```{r pf_aa_v26}

data %>% 
  #mutate(across(starts_with("antiAmerican_"), ~case_when(.x == 0 ~ "Pro-American", .x == 1 ~ "Anti-American"))) %>%
  group_by(antiAmerican_v26, prime) %>%
  drop_na(press_freedom) %>% 
  summarize(avg = mean(press_freedom), se = sd(press_freedom)/sqrt(length(press_freedom))) %>%
  ggplot(aes(x = prime, y = avg, ymin = avg - se, ymax = avg + se, color = antiAmerican_v26)) + 
  geom_errorbar(position = position_dodge(width = 0.4), size=1) +
  labs(x = "", y = "Press Freedom (0-1)", color = "Anti-American? V26") +
  ylim(c(0, 1)) +
  scale_color_manual(
    labels = c("0" = "Pro-American", "1" = "Anti-American"), 
    values = c("#008FB3", "#F36E21")) +
  theme(text = element_text(size=15, family = "Lato", color = "black"), 
        plot.title = element_text(hjust = 0.5, margin = margin(t = 0, r = 0, b = 20, l = 0)),
        axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, color = "black"),
        axis.line = element_line(colour = "black"),
        panel.background = element_rect(fill = "transparent"), # bg of the panel
        panel.grid.major.y = element_line(color = "lightgrey"),
        plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
        legend.background = element_rect(fill = "transparent"), # get rid of legend bg
        legend.box.background = element_rect(fill = "transparent")) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))


```

# Alternative Plotting Methods

## Democrats separated into groups
"Democrats" are those respondents that selected "Strongly agree" or "Agree"
"Non-Democrats" are those respondents that selected "Strongly disagree" or "Disagree"
Note that those who answered "Neither agree or disagree" are dropped. 

Once the "Neither agree or disagree" group is dropped, there are 1518 responses.

```{r pf_d_groups}

data %>% 
  mutate(across(starts_with("democrat_v24_"), ~case_when(
    .x == 5 ~ "Democrat", 
    .x == 4 ~ "Democrat",
    .x == 3 ~ NA_character_,
    .x == 2 ~ "Non-Democrat",
    .x == 1 ~ "Non-Democrat",
    TRUE ~ NA_character_
    ))) %>%
  group_by(democrat_v24_numeric, prime) %>%
  drop_na(press_freedom, democrat_v24_numeric) %>% 
  summarize(avg = mean(press_freedom), se = sd(press_freedom)/sqrt(length(press_freedom))) %>%
  ggplot(aes(x = prime, y = avg, ymin = avg - se, ymax = avg + se, color = democrat_v24_numeric)) + 
  geom_errorbar(position = position_dodge(width = 0.4), size=1) +
  labs(x = "", y = "Press Freedom (0-1)", color = "Democrat? V24 Grouped") +
  ylim(c(0, 1)) +
  scale_color_manual(values = c("#51c2d5", "#ec4646")) +
  theme(text = element_text(size=15, family = "Lato", color = "black"), 
        plot.title = element_text(hjust = 0.5, margin = margin(t = 0, r = 0, b = 20, l = 0)),
        axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, color = "black"),
        axis.line = element_line(colour = "black"),
        panel.background = element_rect(fill = "transparent"), # bg of the panel
        panel.grid.major.y = element_line(color = "lightgrey"),
        plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
        legend.background = element_rect(fill = "transparent"), # get rid of legend bg
        legend.box.background = element_rect(fill = "transparent")) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))

```


##  Violin Density Plots

This shows the density distributions for Democrats and Non-Democrats next to each other. It might help reveal some larger trends of bimodal distributions among our variables. However, it's a bit tricky to interpret because respondents could only select 1, 2, 3, 4 or 5 and not any non-whole values. I just included it as a possibility, but not really my favorite option. 

```{r pf_d_violin}

data %>% 
  mutate(across(starts_with("democrat_"), ~case_when(.x == 0 ~ "Non-Democrat", .x == 1 ~ "Democrat"))) %>%
  group_by(democrat_v24, prime) %>%
  drop_na(press_freedom) %>% 
  ggplot(aes(x = prime, y = press_freedom, fill = democrat_v24)) + 
  geom_split_violin() +
  labs(x = "", y = "Press Freedom (0-1)", fill = "Democrat? V24") +
  #ylim(c(2.5, 3.5)) +
  scale_fill_manual(values = c("#51c2d5", "#ec4646")) +
  theme(text = element_text(size=15, family = "Lato", color = "black"), 
        plot.title = element_text(hjust = 0.5, margin = margin(t = 0, r = 0, b = 20, l = 0)),
        axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, color = "black"),
        axis.line = element_line(colour = "black"),
        panel.background = element_rect(fill = "transparent"), # bg of the panel
        panel.grid.major.y = element_line(color = "lightgrey"),
        plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
        legend.background = element_rect(fill = "transparent"), # get rid of legend bg
        legend.box.background = element_rect(fill = "transparent"))


```
